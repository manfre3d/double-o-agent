...
> **Status: Security hardening on top of Stretch 4 — every visitor is now an isolated anonymous session (signed `__Host-` cookie → `Mission.ownerId`; all mission reads are deny-by-default owner-scoped, closing the OWASP A01 cross-user data leak / IDOR). Adds session-keyed rate limiting on the LLM endpoints, an hourly retention purge, and PDF magic-byte + filename hardening. New required secret `SESSION_COOKIE_SECRET` (Render auto-generates it; see README «Deploy»). Deploy is still the Stretch-4 Render Blueprint (`prisma migrate deploy` on boot). Demo mode is now a runtime per-mission choice, not a boot-time brain swap: both brains coexist when a key is present, HQ's header LIVE/DEMO toggle picks per launch (default live), and `/api/status.llmAvailable` locks the toggle to demo when no key is configured. All roadmap phases + stretch goals built; see ROADMAP.md.** Update this line as phases complete.
